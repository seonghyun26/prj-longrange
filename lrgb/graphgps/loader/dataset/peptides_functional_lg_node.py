...
class PeptidesFunctional_LG_NODE_Dataset(InMemoryDataset):
    def __init__(self, root='datasets', smiles2graph=smiles2graph,
        ...
        self.folder = osp.join(root, 'peptides-functional_lg_node')
    ...
    def process(self):
        ...
        for i in tqdm(range(len(smiles_list))):
            ...
            lg_node_attr = torch.cat([lg_node_attr_node[:, 0, :], lg_node_attr_node[:, 1, :]], dim=1)
            ...
            node1 = x[lg_edge_idx[:, 0, 0]]
            node2 = x[lg_edge_idx[:, 0, 1]]
            node3 = x[lg_edge_idx[:, 1, 1]]
            lg_edge_attr = torch.cat([node1, node2, node3], dim=1)
