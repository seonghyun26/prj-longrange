...
class VOCSuperpixels_lg_vn(InMemoryDataset):
    ...
    def process(self):
        for split in ['train', 'val', 'test']:
            ...
            for idx in indices:
                ...
                # NOTE: add virtual node
                virtualNodeNum = edge_index.shape[1]
                virtualNode = torch.zeros((1, lg_node_attr.shape[1]))
                lg_node_attr = torch.cat([lg_node_attr, virtualNode], dim=0)
                
                incomingEdge = torch.tensor([[2 * i, virtualNodeNum] for i in range(virtualNodeNum//2)])
                outgoingEdge = torch.tensor([[virtualNodeNum, 2 * i + 1] for i in range(virtualNodeNum//2)])
                lg_edge_idx_mask = torch.cat([lg_edge_idx_mask, incomingEdge, outgoingEdge], dim=0)
                lg_edge_attr = torch.cat([lg_edge_attr, torch.zeros([incomingEdge.shape[0] * 2, lg_edge_attr.shape[1]])], dim=0)
